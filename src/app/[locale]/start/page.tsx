...
const steps = [
  {
    ...
    wallets: [
      { name: "Nautilus", type: "Browser Extension", description: "Most popular Ergo wallet with dApp support and advanced features", link: "https://chrome.google.com/webstore/detail/nautilus-wallet/gjlmehlldlphhljhpnlddaodbjjcchai", recommended: true, buttonText: "Download Nautilus", icon: Download },
      { name: "Ergo Mobile", type: "iOS & Android", description: "Official mobile wallet with secure storage and easy transactions", link: "https://apps.apple.com/us/app/terminus-wallet-ergo/id1643137927", recommended: false, buttonText: "Get Ergo Mobile", icon: Smartphone },
      { name: "SAFEW", type: "Web Wallet", description: "Simple web-based wallet for quick access and beginners", link: "https://github.com/ThierryM1212/SAFEW", recommended: false, buttonText: "Open SAFEW", icon: Globe }
    ]
  },
  {
    number: "02",
    ...
    exchanges: [
      { name: "KuCoin", type: "Spot · ~0.1% fees", description: "Large centralized exchange with high liquidity and ERG/USDT pair", link: "https://www.kucoin.com/trade/ERG-USDT?utm_source=ergo_start", fees: "~0.1%", buttonText: "Trade ERG on KuCoin", recommended: false },
      { name: "Gate.io", type: "Spot · ~0.2% fees", description: "Global exchange with multiple ERG trading pairs and good volume", link: "https://www.gate.io/trade/ERG_USDT?utm_source=ergo_start", fees: "~0.2%", buttonText: "Trade ERG on Gate.io", recommended: false },
      { name: "ErgoDEX", type: "No KYC · ~0.3% fees", description: "Native Ergo DEX with full decentralization and privacy", link: "https://app.ergodex.io/?utm_source=ergo_start", fees: "~0.3%", buttonText: "Open ErgoDEX", recommended: false }
    ]
  },
  {
    number: "03",
...
const quickFacts = [
  { icon: Shield, title: "Secure by Design", description: "Non-custodial wallets give you full control of your funds" },
  { icon: CheckCircle, title: "Low Fees", description: "Predictable fees around $0.01 per transaction" },
  { icon: Send, title: "Fast Confirmation", description: "Transactions confirm in approximately 2 minutes" }
]
...
export default async function StartPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const schemas = [
    howToSchema(t),
    knowledgeGraph,
    createBreadcrumbSchema([{ name: "Start", href: "/start" }]),
    ...
    <>
      ...
      <main className="min-h-screen bg-black text-white relative overflow-hidden">
        <BackgroundWrapper>
              ...
              <div className="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="relative z-10">
                  <div className="bg-black/80 border border-white/10 rounded-3xl p-8 hover:bg-black/90 hover:border-orange-400/40 transition-all duration-300">
                    <h3 className="text-2xl font-bold mb-6 text-center text-white">Quick Facts</h3>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {step.wallets.map((wallet, walletIndex) => (
                              <div key={walletIndex} className={`group relative bg-black/80 border border-white/10 rounded-3xl p-8 hover:bg-black/90 hover:border-orange-400/50 transition-all duration-300 hover:-translate-y-0.5 ${wallet.recommended ? 'ring-2 ring-orange-500/20' : ''}`}>
                                {wallet.recommended && <div className="absolute -top-3 left-6 bg-orange-500 text-black px-4 py-2 rounded-full text-sm font-bold">Recommended</div>}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                              {step.exchanges.map((exchange, exchangeIndex) => (
                                <div key={exchangeIndex} className={`group relative bg-black/80 border border-white/10 rounded-3xl p-8 hover:bg-black/90 hover:border-orange-400/50 transition-all duration-300 hover:-translate-y-0.5 ${exchange.recommended ? 'ring-2 ring-orange-500/20' : ''}`}>
                                  {exchange.recommended && <div className="absolute -top-3 left-6 bg-orange-500 text-black px-4 py-2 rounded-full text-sm font-bold">Recommended</div>}
                                  <div className="flex items-start gap-4 mb-4">
                                    <div className="w-12 h-12 rounded-full bg-orange-500/10 border border-orange-500/30 group-hover:bg-orange-500/20 group-hover:border-orange-500/50 transition-all duration-300 flex items-center justify-center flex-shrink-0">
                                  ...
                                  <EnhancedButton href={exchange.link} external={true} variant={exchange.recommended ? "primary" : "secondary"} size="lg" className="w-full" icon={<ShoppingCart className="h-5 w-5" />} ariaLabel={`Trade ERG on ${exchange.name}`}>{exchange.buttonText}</EnhancedButton>
                                </div>
                              ))}
                            </div>
                            <div className="mt-6 p-4 bg-neutral-900/50 border border-neutral-700/50 rounded-lg">
                              <p className="text-xs text-gray-500 text-center"><strong>Disclaimer:</strong> Links are informational. Do your own research. Trading involves risk.</p>
                            </div>
                          </>
                        )}
                        {step.tips && (
          ...
          <section className="py-16">
            <div className="container px-4 md:px-6 mx-auto">
              <div className="max-w-4xl mx-auto text-center">
                <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">What's <span className="text-orange-400">Next</span>?</h2>
                <p className="text-lg text-gray-400 mb-12">Now that you have ERG, explore the Ergo ecosystem</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Link href="/ecosystem" className="group relative bg-black/80 border border-white/10 rounded-3xl p-8 hover:bg-black/90 hover:border-orange-400/50 transition-all duration-300 hover:-translate-y-0.5 text-left">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-12 h-12 rounded-full bg-orange-500/10 border border-orange-500/30 group-hover:bg-orange-500/20 group-hover:border-orange-500/50 transition-all duration-300 flex items-center justify-center flex-shrink-0">
                  ...
                  <Link href="/docs" className="group relative bg-black/80 border border-white/10 rounded-3xl p-8 hover:bg-black/90 hover:border-orange-400/50 transition-all duration-300 hover:-translate-y-0.5 text-left">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-12 h-12 rounded-full bg-orange-500/10 border border-orange-500/30 group-hover:bg-orange-500/20 group-hover:border-orange-500/50 transition-all duration-300 flex items-center justify-center flex-shrink-0">
                        <Shield className="h-6 w-6 text-orange-400" />
                      </div>
          ...
          <FinalCTASimple title="Get Started with Ergo" description="Get the latest updates, beginner guides, and step-by-step tutorials delivered to your inbox" />
        </BackgroundWrapper>
      </main>
    </>
  )

...
export default function AppkitTutorialPage() {
  return (
    <>
      ...
      <div className="mb-6">
        <Link
          href="/docs/developers/tooling/appkit"
      ...
      <CodeBlock language="typescript">{`// Freezer Contract
{ 
  // Parameters
  ...
  "parameters": {
    ...
    ...
      ...
      <CodeBlock language="typescript">{`int newBoxSpendingDelay = Integer.parseInt(conf.getParameters().get("newBoxSpendingDelay"));
Address ownerAddress = Address.create(conf.getParameters().get("ownerAddress"));`}</CodeBlock>
...
ErgoClient ergoClient = RestApiErgoClient.create(nodeConf);`}</CodeBlock>
    ...
    // then send it to the node 
      ...
      <CodeBlock language="typescript">{`// access the wallet embedded in the Ergo node 
...
ErgoProver prover = ctx.newProverBuilder()
    ...
    .build();`}</CodeBlock>
      ...
      <CodeBlock language="typescript">{`// the only way to create a transaction is using the tx builder obtained from the context
...
OutBox newBox = txB.outBoxBuilder()
      ...
      <CodeBlock language="typescript">{`// provide the transaction builder with which boxes we are going to spend, which outputs
...
return signed.toJson(true);`}</CodeBlock>
      ...
      <CodeBlock language="typescript">{`$ /usr/bin/time -l java -cp build/libs/appkit-examples-3.1.0-all.jar \
   org.ergoplatform.appkit.examples.FreezeCoin 1000000000
...`}</CodeBlock>
      <p className="text-gray-300 mb-2">With GraalVM:</p>
      <CodeBlock language="typescript">{`$ ./gradlew clean shadowJar
 ...
 --report-unsupported-elements-at-runtime\
  --no-fallback -H:+TraceClassInitialization -H:+ReportExceptionStackTraces\
   ...
   --allow-incomplete-classpath \
    --enable-url-protocols=http,https org.ergoplatform.appkit.examples.FreezeCoin freezecoin`}</CodeBlock>
      <CodeBlock language="typescript">{`$ DYLD_LIBRARY_PATH=$GRAAL_HOME/jre/lib /usr/bin/time -l ./freezecoin 1800000000
  ...
  81289216  maximum resident set size
     ...
     20079  page reclaims
       ...
       138  involuntary context switches`}</CodeBlock>
      ...
      <CodeBlock language="typescript">{`$ node --jvm --vm.cp=build/libs/appkit-examples-3.1.0-all.jar \
  js-examples/FreezeCoin.js  1000000000`}</CodeBlock>
      <h4 className="text-lg font-bold text-cyan-400 mt-4 mb-2">Python</h4>
      <CodeBlock language="typescript">{`$ graalpython --jvm --polyglot --vm.cp=build/libs/appkit-examples-3.1.0-all.jar \
   python-examples/FreezeCoin.py 1900000000`}</CodeBlock>
      <h4 className="text-lg font-bold text-cyan-400 mt-4 mb-2">Ruby</h4>
      <CodeBlock language="typescript">{`$ truffleruby --polyglot --jvm --vm.cp=build/libs/appkit-examples-3.1.0-all.jar \
    ruby-examples/FreezeCoin.rb 1900000000`}</CodeBlock>
      ...
      <CodeBlock language="typescript">{`public class FreezeCoin {
    ...
     /**
    ...
    ...
      ...
      <CodeBlock language="typescript">{`$ native-image --no-server \
 -cp build/libs/appkit-examples-3.1.0-all.jar\
 --report-unsupported-elements-at-runtime\
  --no-fallback -H:+TraceClassInitialization -H:+ReportExceptionStackTraces\
   -H:+AddAllCharsets -H:+AllowVMInspection -H:-RuntimeAssertions\
   --allow-incomplete-classpath \
    ...
    --shared -H:Name=libfreezecoin -H:Path=c-examples`}</CodeBlock>
      ...
      <CodeBlock language="typescript">{`#include <stdlib.h>
#include <stdio.h>
...
int main(int argc, char **argv) {
  ...
  return 0;
      ...
      <CodeBlock language="typescript">{`$ clang -Ic-examples -Lc-examples -lfreezecoin c-examples/freezecoin.c -o call_freezecoin
$ otool -L call_freezecoin
$ DYLD_LIBRARY_PATH=$GRAAL_HOME/jre/lib ./call_freezecoin 1000000000`}</CodeBlock>
      ...
      <CodeBlock language="typescript">{`$ ruby --polyglot --jvm --inspect --vm.cp=build/libs/appkit-examples-3.1.0-all.jar \

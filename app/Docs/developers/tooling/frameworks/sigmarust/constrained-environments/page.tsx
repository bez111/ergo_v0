...
export default function ConstrainedEnvironmentsPage() {
  return (
    <>
      <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-orange-400 to-cyan-400 bg-clip-text text-transparent mb-4 leading-tight pb-1">Using Sigma-Rust in Resource-Constrained Environments</h1>
      <div className="mb-6">
        <Link
          href="/docs/developers/tooling/frameworks/sigmarust"
      ...
      <p className="text-lg text-gray-300 mb-6 max-w-2xl">
        The standard <b>sigma-rust</b> library provides comprehensive tools for working with Ergo protocols and data structures in Rust. However, environments like hardware wallets or embedded systems often have strict limitations on code size, memory usage, and available libraries (especially the standard library, <code>std</code>).
      </p>
      <p className="text-gray-300 mb-6 max-w-2xl">
        This guide outlines strategies and considerations for adapting <b>sigma-rust</b> for such resource-constrained environments.
      </p>
      <h2 className="text-2xl font-bold text-cyan-400 mb-2">Challenges</h2>
      <ul className="list-disc pl-6 text-gray-300 mb-6 space-y-1">
        <li><b>Code Size:</b> The full <b>sigma-rust</b> library can be relatively large due to its extensive features and dependencies.</li>
      ...
      <h2 className="text-2xl font-bold text-cyan-400 mb-2">Solutions & Approaches</h2>
      <h3 className="text-xl font-semibold text-orange-400 mt-4 mb-2">1. sigma-rust-mini Fork</h3>
      <ul className="list-disc pl-6 text-gray-300 mb-4 space-y-1">
        ...
        <li><b>Repository:</b> <a href="https://github.com/Alesfatalis/sigma-rust-mini/tree/no_std" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">github.com/Alesfatalis/sigma-rust-mini/tree/no_std</a></li>
        ...
        <li><b>Considerations:</b> Might lag behind the main <b>sigma-rust</b> repository in terms of features or updates. Verify its maintenance status and compatibility with your required <b>sigma-rust</b> version.</li>
        <li><b>When to Use:</b> Often the easiest starting point for hardware wallet integration or <code>no_std</code> projects.</li>
      </ul>
      <h3 className="text-xl font-semibold text-orange-400 mt-4 mb-2">2. no_std Builds (Manual Configuration)</h3>
      <ul className="list-disc pl-6 text-gray-300 mb-4 space-y-1">
        <li><b>Concept:</b> Attempt to compile the main <b>sigma-rust</b> library (or a specific subset) with the <code>no_std</code> feature flag enabled, potentially requiring manual adjustments to dependencies and features.</li>
        ...
        <li><b>Challenges:</b> Can be complex, as not all features or dependencies of the main <b>sigma-rust</b> library might be <code>no_std</code> compatible. Requires careful dependency management.</li>
        <li><b>When to Use:</b> If <b>sigma-rust-mini</b> is unsuitable or outdated, or if you need fine-grained control over included features.</li>
      </ul>
      <h3 className="text-xl font-semibold text-orange-400 mt-4 mb-2">3. Replacing the Cryptographic Backend (k256 -&gt; secp256k1)</h3>
      <ul className="list-disc pl-6 text-gray-300 mb-4 space-y-1">
        <li><b>Problem:</b> The default <code>k256</code> crate used by <b>sigma-rust</b> for elliptic curve operations can be large or have <code>std</code>-dependent features. Hardware wallets often use the more lightweight, C-based <code>secp256k1</code> library (via the <code>secp256k1</code> Rust crate).</li>
        <li><b>Solution:</b> Modify <b>sigma-rust</b> (or <b>sigma-rust-mini</b>) to use the <code>secp256k1</code> crate as the backend for cryptographic operations instead of <code>k256</code>.</li>
          ...
          <ol className="list-decimal pl-6 text-gray-300 mb-2 space-y-1">
            ...
            <li><b>Code Adaptation:</b> Search the codebase for usages of <code>k256</code> types and functions and replace them with their <code>secp256k1</code> equivalents. Key areas include:
              <ul className="list-disc pl-6 text-gray-300 mb-2 space-y-1">
                ...
                <li><b>Point Operations:</b> Operations like point multiplication (<code>mul_tweak</code>) or addition (<code>combine</code>) needed for Diffie-Hellman proofs or signature aggregation must use the <code>secp256k1</code> crate's functions. (Refer to the <a href="https://docs.rs/secp256k1/latest/secp256k1/" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">secp256k1 crate documentation</a> for specific methods).</li>
              </ul>
            </li>
            <li><b>Feature Flags:</b> Ensure appropriate feature flags are enabled for both <b>sigma-rust</b> and <b>secp256k1</b> to support the required operations in a <code>no_std</code> context.</li>
          </ol>
        </li>
      ...
      <h2 className="text-2xl font-bold text-cyan-400 mb-2">Potential Pitfalls</h2>
      <ul className="list-disc pl-6 text-gray-300 mb-6 space-y-1">
        ...
        <li><b>Feature Creep:</b> Be mindful of enabling features in <b>sigma-rust</b> or its dependencies that might pull in <code>std</code> unexpectedly. Start with minimal features and add only what is necessary.</li>
      ...
      <h2 className="text-2xl font-bold text-cyan-400 mb-2">Conclusion</h2>
      <p className="text-gray-300 mb-8 max-w-2xl">
        Adapting <b>sigma-rust</b> for resource-constrained environments is feasible but requires careful planning. Starting with <b>sigma-rust-mini</b> is often the recommended approach. If modifications are needed, replacing the cryptographic backend with the <code>secp256k1</code> crate is a common and necessary step, particularly for hardware wallet integration. Always prioritize thorough testing on the target platform.
      </p>
    </>
  );
} 
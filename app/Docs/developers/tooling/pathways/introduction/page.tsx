...
import { BookOpen, Key, Box, Globe, MessageCircle, HelpCircle, TestTube, Link2, Users, Cpu, Code, Layers, Wrench, FileText, Database, Zap, Package, FileQuestion, ExternalLink } from "lucide-react";

const resourcesCards = [
  {
    title: "Key Features",
    content: "Explore the most important features that make Ergo unique: advanced scripting, storage rent, privacy, and more.",
    icon: <Key className="w-8 h-8 text-yellow-400" />,
    url: "/Docs/developers/infrastructure/node"
  },
  {
    title: "Anatomy",
    content: "Understand the structure of Ergo: boxes, registers, tokens, and how data is stored on-chain.",
    icon: <Box className="w-8 h-8 text-cyan-400" />,
    url: "../data-model/box.md"
  },
  {
    title: "Read our FAQ",
    content: "Answers to common questions about Ergo, wallets, development, and ecosystem.",
    icon: <HelpCircle className="w-8 h-8 text-orange-400" />,
    url: "/Docs/introduction/faq"
  },
  {
    title: "Testnet",
    content: "A public test blockchain for safe experimentation and development before mainnet deployment.",
    icon: <TestTube className="w-8 h-8 text-green-400" />,
    url: "/Docs/developers/infrastructure/node/testnet"
  },
  // Community cards moved below
  {
    title: "Come chat!",
    content: "Join the Ergo developer community on Discord for support, discussion, and hackathons.",
...
const frameworks = [
  { title: "AppKit", content: "JVM-based SDK for building Ergo dApps in Scala, Java, or Kotlin. Provides full blockchain and transaction access.", icon: <Cpu className="w-8 h-8 text-blue-400" />, url: "/Docs/developers/tooling/pathways/appkit" },
  { title: "Fleet", content: "Modern TypeScript/JavaScript library for creating and signing Ergo transactions in web and Node.js apps.", icon: <Code className="w-8 h-8 text-green-400" />, url: "/Docs/developers/tooling/pathways/fleet" },
  { title: "Mosaik", content: "JSON-based UI framework for building cross-platform dApps, rendered natively in supported wallets.", icon: <Layers className="w-8 h-8 text-orange-400" />, url: "/Docs/developers/tooling/pathways/mosaik" },
  { title: "Headless dApp Framework", content: "Rust toolkit for building backend-only (headless) dApps and off-chain logic.", icon: <Wrench className="w-8 h-8 text-yellow-400" />, url: "/Docs/developers/tooling/pathways/headless" },
  { title: "RustKit", content: "(WIP) High-level Rust library for composing and signing Ergo transactions, built on sigma-rust.", icon: <Wrench className="w-8 h-8 text-yellow-400" />, url: "/Docs/developers/tooling/pathways/rustkit" },
  { title: "JDE", content: "JSON dApp Environment: a lightweight environment for running JSON-based dApps.", icon: <FileText className="w-8 h-8 text-cyan-400" />, url: "jde.md" }
];

const interpreters = [
  { title: "sigmastate-interpreter", content: "Reference JVM implementation of ErgoScript and ErgoTree, used in the core node and AppKit.", icon: <Cpu className="w-8 h-8 text-blue-400" />, url: "../lang/jvm.md" },
  { title: "Sigma-Rust", content: "Alternative Rust implementation of ErgoTree interpreter and transaction tools. Useful for cross-platform and WASM.", icon: <Wrench className="w-8 h-8 text-yellow-400" />, url: "sigma-rust.md" },
  { title: "Sigma.JS", content: "JavaScript/TypeScript library for parsing and working with ErgoTree scripts in web apps.", icon: <Code className="w-8 h-8 text-green-400" />, url: "../lib/sigmajs.md" }
];
...
function CardGrid({ items, icon }: { items: any[]; icon?: boolean }) {
  return (
  ...
  return (
    <>
      ...
      <p className="text-lg text-gray-300 mb-8">
        Overview of Ergo development and ecosystem.
      </p>
      <h2 className="text-2xl font-bold text-cyan-400 mb-6 flex items-center">Resources</h2>
      <CardGrid items={baseResources} icon />

      <h2 className="text-2xl font-bold text-cyan-400 mb-6 flex items-center">Introductory</h2>
      <h3 className="text-xl font-semibold text-orange-400 mb-3">Overview Articles</h3>
      <SimpleList items={overviewArticles} />
      <h3 className="text-xl font-semibold text-orange-400 mb-3">Dev-Articles</h3>
      <SimpleList items={devArticles} />
      <h3 className="text-xl font-semibold text-orange-400 mb-3">Courses</h3>
      <CardGrid items={courses} />

      <h2 className="text-2xl font-bold text-cyan-400 mb-6 flex items-center">Frameworks</h2>
      <CardGrid items={frameworks} icon />

      <h2 className="text-2xl font-bold text-cyan-400 mb-6 flex items-center">ErgoScript Interpreter/Compilers</h2>
      <CardGrid items={interpreters} icon />

      <h2 className="text-2xl font-bold text-cyan-400 mb-6 flex items-center">Libraries</h2>
      <CardGrid items={libraries} />

      <h2 className="text-2xl font-bold text-cyan-400 mb-6 flex items-center">Simple Examples</h2>
      <CardGrid items={simpleExamples} />

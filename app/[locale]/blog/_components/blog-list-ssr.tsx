        ...
        {posts.map((p) => (
          <li key={p.id} role="listitem">
            ...
            >
              ...
              <time dateTime={new Date(p.date).toISOString()} className="text-neutral-400 text-sm">
                {p.date}
              </time>
              <meta itemProp="datePublished" content={new Date(p.date).toISOString()} />
              <meta itemProp="dateModified" content={new Date(p.date).toISOString()} />

...
const agentsHostRoutes = [
  ...
  ['/api/reputation', '/api/agents/reputation'],
  ['/api/publish', '/api/agents/publish'],
  ['/api/jobs', '/api/jobs'],
  ['/api/accept', '/api/jobs/accept'],
  ['/api/quote', '/api/jobs/quote'],
...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    const agentsHostRewrites = agentsHostRoutes.map(([source, destination]) => ({
      source,
      has: [{ type: 'host' as const, value: agentsHost }],
      destination,
    }));

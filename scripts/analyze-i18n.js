...
function loadLocaleMessages(messagesDir, locale) {
  const flatPath = path.join(messagesDir, `${locale}.json`);
  ...
  for (const namespaceFile of namespaces) {
    ...
    const namespacePath = path.join(localeDir, namespaceFile);
    data[namespace] = JSON.parse(fs.readFileSync(namespacePath, 'utf8'));
  }

  return data;
}

function getLocaleCodes(messagesDir) {
  return fs
...
function analyzeI18n() {
  ...
  const locales = getLocaleCodes(messagesDir);

  console.log('🌍 АНАЛИЗ ЛОКАЛИЗАЦИЙ\n');
  console.log('='.repeat(60));

  // Загружаем эталонный английский файл
  const enData = loadLocaleMessages(messagesDir, 'en');
  ...
  locales.forEach(lang => {
    ...
    const langData = loadLocaleMessages(messagesDir, lang);
...
module.exports = { analyzeI18n, getAllKeys, getLocaleCodes, loadLocaleMessages };
